...
**Scope:** Delivery of finalized vote-stage USDC refunds and rewards from an Opal Vote Settlement Vault into participants' MagicBlock Private Voting Balances.

## Decision

The Devnet MVP uses **participant-claimed payouts**:

1. Finalize the Vote Round and freeze its payout formula without enumerating every recipient.
...
5. Keep unclaimed USDC reserved in the Vote Settlement Vault without a forfeiture deadline.

Only positive payouts become `Claimable`. A fully slashed position is terminal `Slashed` and requires no zero-value claim.

The round's truth outcome is final before every payout is claimed. `Resolved` therefore means that the outcome and payout formula are immutable; it does not mean every participant has received USDC. Positive payouts become claimable immediately when finalization confirms, without a cooldown or claim-opening transaction. Their amounts never change afterward and do not accrue while unclaimed.
...
**Inference for Opal:** finalizing the outcome can be atomic, and each individual claim can be atomic, but paying an unlimited voter set cannot be one atomic action. Required claims distribute the necessary `O(number of positions)` work across recipients.
...
### Private-balance delivery has prerequisites, but Opal already satisfies them
...
**Inference for Opal:** the generic Payments API is not the protocol accounting layer. `claim_vote_payout` should use Opal's program-controlled vault authority and an internal same-PER token transfer. The payout owner signs the Opal claim, while Opal's PDA authorizes the vault debit. Every voter already pre-funds a Private Voting Balance, and Vote Initialization sponsors one for any bonded participant who lacks it, so terminal claim processing does not need to create the destination.
...
**Inference for Opal:** queued private transfers solve delayed or cross-runtime delivery. Opal's Vote Settlement Vault and Private Voting Balances are already co-located in one PER, and the MVP publishes wallet-to-vote mappings after finalization. A direct synchronous same-PER claim avoids queue callbacks, refund states, and a second asynchronous workflow. A participant's later withdrawal to Solana remains a separate wallet-authorized cross-runtime operation.

## Proposed Devnet state and instruction model

### 1. Finalize the round once

The finalization instruction should atomically freeze:

- final outcome and quorum/supermajority evidence;
- final aggregate stake totals;
- fee and Slashed Pool totals;
- the immutable pro-rata numerator and denominator inputs;
- the immutable number of voter positions;
- the total outstanding payout liability.

It should not attempt to enumerate all recipients. Integrators may treat the truth result as final after this transaction, independently of payout claim or public-position publication progress. The target representation uses one permanent public `VotePositionAccount` per Vote Round and voter wallet, with only aggregates and publication counters on the round; see [ADR-0017](../adr/0017-per-voter-public-position-accounts.md).
...
- transfer the payout from the Vote Settlement Vault;
- mark the position claimed and reduce outstanding liability in the same transaction;
- leave the credited USDC immediately unlocked for another Vote or withdrawal.

Because Solana transactions are atomic, a failed claim leaves the position and vault unchanged and safe to retry. ([Solana transactions](https://solana.com/docs/core/transactions))
...
The Vote Settlement Vault may close only when every position is claimed, its outstanding payout liability is zero, and accounting reconciliation succeeds. One invalid claim must not affect any other position. No forfeiture deadline is recommended for Devnet.
...
5. Leave claims outstanding; their exact liability remains reserved and the vault cannot close.
...
8. Settle every position, reconcile the vault, and close only after outstanding liability reaches zero.
