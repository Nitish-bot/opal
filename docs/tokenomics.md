...
Opal uses a **single asset: USDC** `[MVP-target]`. The same token is collateral for assertion bonds, dispute bonds, voting stake, slashing, rewards, treasury fees, and indivisible pro-rata dust. There is no second protocol token in the MVP — governance is the `authority` keypair, and voting weight is staked USDC, not a governance token. See [ADR-0004](adr/0004-single-asset-usdc.md).
...
USDC is used for assertion bonds, the first (LLM) dispute bond, the second (vote) dispute bond, voting stake, slashed collateral, disputer and voter rewards, protocol treasury fees, and indivisible pro-rata dust. The mint is a config field (`pusd_mint`, to become `usdc_mint`) so localnet/devnet can run against a test mint, but the protocol commits to USDC — we do **not** support "any USD-pegged stablecoin." See [ADR-0004](adr/0004-single-asset-usdc.md).
...
The vote disputer posts 500 USDC to challenge the LLM verdict and trigger private-vote initialization. It also pre-funds a zero-data, System-owned `VoteSetupPayer` PDA with the configured capped lamports for fixed onchain account-rent and delegation reserves; those SOL reserves are separate from the equal USDC Participant Bond. Each later caller pays its own transaction/provider fees. The permissionless retry model is a release gate on proving that the selected low-level MagicBlock CPI accepts the Opal-signed payer PDA. The posted bond is stored as `VoteDisputeAccount.bond_amount_pusd`.
...
Opal charges no protocol fee on Private Voting Balance deposits, payout claims, or withdrawals. External network or provider execution costs may still apply and are separate from the Bond Fee and Voting Fee. Opal provides no protocol-funded sponsorship: participants and transaction callers pay their assigned costs unless an integrating application voluntarily sponsors them outside the protocol guarantee.
...
On official Devnet, Bond Fees, Voting Fees, and treasury-owned pro-rata dust transfer to a USDC treasury token account owned by the same Opal-team-controlled single-signature wallet that controls program upgrades and `ProtocolConfig.authority`. All three vote-stage Bond Fees and the exact accumulated total of independently floored Voting Fees first move to the round's program-PDA-controlled `TreasuryStagingBalance`, then a fixed-destination `TreasuryFeeDelivery` operation moves exactly the recorded fee liability to the public account; the operation is permissionless only if the feasibility spike proves the selected eSPL route accepts Opal's PDA authority. Once the positive-unclaimed-position count reaches zero, checked reconciliation classifies the remaining bounded residual as dust and sends it through a distinct `TreasuryDustDelivery`. Each record has its own nonce and one-way status; neither is reset for the other tranche. Public destination confirmation completes each delivery. Once funds leave the logical Vote Settlement Vault, a pending delivery blocks closing the staging balance and that delivery record rather than the emptied settlement record and does not reopen the truth outcome. The wallet may move delivered treasury USDC at any time; Devnet has no multisig, timelock, withdrawal cap, or program-governed spending policy. This is a disclosed Devnet trust choice, not a Mainnet commitment. Exact authority and treasury addresses are published only after the completed deployment is designated official. See [ADR-0018](adr/0018-devnet-single-admin-and-treasury-authority.md).
...
If either Vote Quorum condition fails, or quorum passes but none of `True`, `False`, or `Unresolvable` reaches the vote's 67% supermajority:
...
If the vote disputer was correct (`settlement_resolution != challenged_llm_resolution`), their bond is refunded after its Bond Fee and contributes `500_000_000` USDC atomic units of Reward Weight. If incorrect, its remaining 495 USDC enters the Slashed Pool. `NoConsensus` assigns no correctness and follows the no-fault path.

## Pro-rata Vote Rewards `[MVP-target]`

Before the Voting Window starts, retryable Vote Initialization deposits all three Participant Bonds—1,500 USDC total on Devnet—from the public Bond Vault into eSPL's shared per-mint global vault on Solana and credits a round-scoped delegated eATA/balance record, Opal's logical **Vote Settlement Vault** inside the PER. Each accepted Vote later atomically moves stake from a Private Voting Balance into that same logical balance. The Vote Disputer pre-funds configured capped onchain rent and delegation reserves through the System-owned `VoteSetupPayer` PDA; each later caller pays its own transaction/provider fees, and Opal does not sponsor them. Every voter creates, funds, and delegates its own canonical Private Voting Balance before casting. A bonded non-voter may defer balance creation until it needs to claim, so its missing destination never blocks initialization or finalization; the MVP requires a spike-proven direct initializer for an empty destination that needs no positive USDC deposit merely to receive a payout. This accounting co-location lets Opal apply one Slashed Pool and one pro-rata calculation without cross-runtime partial payouts.
...
Correct bonded participants become entitled to their 495 USDC refunds. Winning voters become entitled to their stake minus the Voting Fee. Every positive vote-stage refund and reward, including a bonded participant's payout, must be claimed into the recipient's existing canonical Private Voting Balance; a claim validates but does not create or redirect that destination. A missing balance does not change the non-expiring `Claimable` entitlement. Claim eligibility begins when authoritative PER settlement freezes the round and can precede the deterministic Solana `Resolved` projection. Inside the PER, the payout transfer, positive-unclaimed-position count, and authoritative private claimed marker update atomically; a later public status projection may lag without permitting a second payment. Vote finalization does not automatically transfer the payout or withdraw it to the public Solana wallet. An incorrect bond or losing Vote is terminal `Slashed` and has no payout to claim. The remaining collateral from every incorrect bond and losing vote forms the **Slashed Pool**.
...
```text
bonded_participant_reward_weight_atomic = 500_000_000
voter_reward_weight_atomic = gross_vote_stake_atomic

participant_reward_atomic = floor(
  slashed_pool_atomic × participant_reward_weight_atomic
  ÷ total_reward_weight_atomic
)

treasury_dust_atomic =
  slashed_pool_atomic - sum(participant_reward_atomic)
```

Every multiplication and sum uses checked `u128` intermediates, and each final USDC amount is range-checked before conversion to `u64`. Aggregate weight alone cannot reveal the sum of all independently floored participant rewards without enumerating positions. Finalization instead freezes per-outcome position counts and initializes the exact number of positive claims. Each successful claim records its exact payout and decrements that count. Once it reaches zero, checked reconciliation against the remaining logical settlement balance classifies a non-negative residual smaller than the number of rewarded positions as indivisible pro-rata dust for the treasury; any larger or otherwise inconsistent residual becomes `RecoveryNeeded` rather than being swept. Dust is not a discretionary percentage of the Slashed Pool. An `Unresolvable` supermajority settles like any other winning voter outcome. `NoConsensus` creates no Slashed Pool and pays no rewards; every fee-adjusted bond and voting stake becomes claimable by its owner. A Vote Round also resolves `NoConsensus` when either quorum condition fails, including when no Votes are cast. In a zero-Vote round, all three bonds become claimable minus their Bond Fees and there is no Voting Fee because there is no voting stake. Every claim amount is fixed permanently at finalization and does not accrue while unclaimed. Claiming moves the payout into the owner's Private Voting Balance and carries no additional Opal fee, although the claimant pays any network execution cost not sponsored by an integrating application. Withdrawing any available private balance to Solana is another separate, manual action.
...
Voting stake must come from a pre-funded **Private Voting Balance**, not a public USDC transfer performed as part of casting. Each deposit must be at least 1 USDC and adds to the same reusable balance. Funding is independent of any one Vote Round: a vote locks only its chosen stake, while the remaining unlocked balance can fund another vote or be withdrawn. Active voting stake cannot be withdrawn. After finalization, any fee-adjusted stake and rewards remain in the logical Vote Settlement Vault balance until the voter claims them into the Private Voting Balance. Once the claim confirms, those funds are immediately unlocked for another Vote or withdrawal; there is no post-claim cooldown. The voter may authorize a withdrawal of any positive amount up to the unlocked balance; Opal imposes no withdrawal minimum. A zero, unlocked Private Voting Balance may be closed even while the wallet owns an outstanding round claim; that claim survives and requires recreating the same canonical empty destination before collection. The balance, positions, and claims remain bound to the original wallet, and Opal cannot recover or redirect them if the wallet's signing key is lost. Deposits and withdrawals are visible on Solana and can still leak timing or amount correlations. Each transfer is `Pending` until the destination balance confirms, then `Completed`; a transfer whose normal path fails has machine status `RecoveryNeeded`, rendered to participants as “Recovery Needed.” Source-transaction confirmation alone does not change the available balance.

The intended privacy behavior is contingent on the PER/eSPL feasibility gate: during the Voting Window, the target MagicBlock PER hides individual choices, stake amounts, private balances, and per-outcome totals. If the spike proves write-without-read authorization and the other required PDA authorities, sealing removes the visible running tally so voters cannot bandwagon onto a leader. PER settlement freezes aggregate stake per outcome, and the base Assertion cannot become `Resolved` until an authenticated projection makes those aggregates public on Solana. Wallet-level outcome, stake, exact payout, and `Claimable`, already-`Claimed`, or `Slashed` status become public only as bounded post-finalization publication creates each permanent `VotePositionAccount`; a later claim is synchronized separately, and the public mapping remains incomplete until `PositionPublicationComplete`. Total Private Voting Balances remain private. Publication is permissionless only if the spike proves its authority path; the caller normally pays its external transaction cost unless an integration sponsors it, while Opal charges no fee and pays no reward. See [ADR-0017](adr/0017-per-voter-public-position-accounts.md). If the Vote Quorum is met, `True`, `False`, or `Unresolvable` must reach 67%; if quorum fails or none reaches 67%, the vote settles `NoConsensus`. See [ADR-0003](adr/0003-private-staked-voting.md) and [ADR-0007](adr/0007-unresolvable-vs-no-consensus.md).
...
- **Source/truth-change invalidation** — a future protocol mechanism that invalidates a non-final Assertion without assigning a truth outcome when authoritative source information or rubric-relative truth materially changes. Its detection, lifecycle representation, and settlement economics remain undecided.
